...
imagemetrics %>% group_by(metric) %>% nest() %>%
    mutate(msplinemodel = map(data, fit_mspline),
           ...
           varMOS = map_dbl(data, ~ var(.x$mean)),
...
fit_revmspline <- function(df, k = 10, lower = NA, upper = NA) {
    mspline(y = df[["score"]], x = df[["mean"]],
            k = k, lower = lower, upper = upper)
}

fit_revmgcv <- function(df, k = 10, ...) {
     y <- df[["score"]]
     x <- df[["mean"]]
     pass_df <- data.frame(x = x, y = y)
    mgcv::gam(y ~ 1+ s(x, k = k, bs = "cr"), data = pass_df,...)
...
knitr::kable(x = revmetricmodels %>% ungroup() %>% select(label, mgcvDevExp, msplineDevExp),
             col.names = c("Metric", "GAM % Deviance", "mono-GAM % Deviance"),
             digits = 3)
```
...
par(mfrow = c(1,2))
for (i in 1:8) {
    plot(revmetricmodels$msplinemodel[[i]], ylab = revmetricmodels$label[i],
